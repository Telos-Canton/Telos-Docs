...
修订后的Telos注册生产者合约将明确规定被禁止的行为以及违规处罚——例如优先处理交易，并将要求所有其他区块生产节点执行。 为了促进这一点，将有一个智能合约约定任何BP都可以执行记录涉嫌违规的链条，并要求所有其他区块生产者投票决定是否执行所需的罚款。 如果2/3 + 1多数票投票执行，则智能合约将立即执行。 无需其他操作。

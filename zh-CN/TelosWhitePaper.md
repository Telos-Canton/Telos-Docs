...
由于来自希望收到通证的客户的压力，交易所可能会期待上线TLOS，从而提供买卖方法。

Telos网络有望更加稳定，因为BP是基于能力由广泛的社区投票选举出的，而不是基于少数巨鲸的联盟。 另外，未能生产区块（180个错过的区块）的BP将在30分钟内轮换出来，而不是3个小时。 此外，预备BP将被启用，并且需要每3-7天生成一个区块以证明它们时刻准备就绪。 这种半定期计划为BP提供了维护和更新服务器所需的停机时间，从而降低了运营成本。 它还把更多的预备BP加入到治理中，因为预备BP的投票将影响BP投票。 这种方法意味着可以验证预备BP的能力和得到更高的网络弹性（可能减少网络暂停）。

开发人员可能更喜欢Telos系统，因为他们可能将某些DApp指定为专有，因此值得他们进行开发投资。 Telos网络将致力于降低部署成本保持RAM的可用性与网络需求相匹配，以减少早期参与者的RAM囤积情况。 这应该使部署DApps的成本低于Eos。 在所有其他方面，Telos网络将与Eos主网软件保持兼容，以便DApps可以在任一网络上互换运行。

通证持有者可以期待能够更多的参与，因为与Eos相比，他们的投票在Telos更为重要。在Eos只有巨鲸的选票才真的重要。 像Weiss Ratings这样的分析师可能会给予Telos更高的估值，因为他们已经警告集中化是Eos系统的一个严重问题，并建议对大型账户进行限制投票和合并更多的BP - 这两者都是Telos做到了。 TLOS通证供应将是EOS的五分之一到三分之一，具体取决于交易所参与情况。 鉴于所有这些优势，TLOS有望比EOS具有更大的价值。
...
所有Telos BP和预备BP都必须在BP专用测试网上有一个节点，在实施之前可以测试新的补丁*。 这通过在实施之前披露新的故障来增加网络安全性，并允许BP之间更好的更新协调以减少死链事件。
...
Telos创世令牌所有权限制了所有创世纪账户上限为40,000个TLOS通证。 Telos基金会拥有的地址不能投票。 因此，Telos通证所有权是任何加密货币网络中最均衡比例的其中一个。 与Eos成员相比，Telos成员在网络治理方面拥有更为平等的发言权。

### 每个通证拥有更高的权益

每个EOS通证代表Eos网络总值的1 / 1,000,0000,000。 由于Telos具有较少的通证，因此每个TLOS通证代表Telos网络值的大约1 / 330,000,000 *。 相同数量的通证的高出网络所有权百分比的3倍以上。

（*取决于激活TLOS通证并分配给EOS通证持有者的交易所的总数量。）

### 开发者专注建立价值

任何基于Eos的网络的价值最终都将反映网络对开发者部署DApps的可用性。 Telos通过降低DApp部署成本和部署专有代码的能力为开发人员提供额外的价值。 随着时间的推移，这些功能有望使Telos网络对开发人员更具吸引力和更有价值。

### 更低的通货膨胀率

Eos通胀率定为每年5％。 Telos的目标是每年通胀率2.5％。 TLOS通证通胀率将比EOS低差不多50％。

### 被盗通证的预激活解决方案

Telos基金会和ABP将提供一份智能合约，允许任何被盗的EOS持有人在Telos主网启动之前证明其所有权。 这会将通证返还给他们的合法所有者，并防止其他成员无意中购买可能因盗窃而被冻结的通证。

* * *

## 对于BP

BP验证交易，提供网络安全性，并在区块链上扮演关键的治理角色。 为了吸引最好的BP加入Telos网络，我们已经实施了以下改进：

### 公平的支付模式

Telos的BP系统由21个BP和30个预备BP组成，根据用户投票排名组成。 每个区块生产者将获得与Telos 1％通胀相同的每日份额。 每个预备区块生产者（最多30个）将获得前21个BP收到的一半金额。 每个预备BP将每3-7天轮换一次区块生产以“获得存在的价值”并证明他们可以不断地履行其职责。 对于任何错过的区块，区块生产者将受到处罚，并将向Telos选民提供统计数据。

### 公平的投票/反投票权重

在Eos投票系统中，巨鲸可以推选他们支持的任何BP。 那些想要只支持他们自身附属的BP的选民可以通过不投票给任何人来增加自己的选票价值 ，而这会伤害投票系统。 Telos没有巨鲸，因此BP只需要用很强的候选资格说服广泛的TLOS通证持有者群体。

此外，Telos BP投票是“反向加权”，这意味着如果您只投票给1或2个BP，那么你的投票权重将低于您投票给30个BP的投票权。 这鼓励大多数Telos成员去了解更多关于BP候选人，并阻止那些只为自己及其分支机构投票的人。
...
Telos维持BP的最低标准，使其成为合格的候选人。 这些标准基于安全性，服务器容量，信息披露以及参与公共和专有测试网。 无论投票情况如何，从主网激活时起，这些标准将由智能合约强制执行，并且任何不符合要求的BP候选人将无法成为BP。 所有其他BP候选人将自动上升等级填补队列，直到满足最低要求。

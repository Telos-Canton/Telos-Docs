...
Eos预备节点的报酬远低于排名前21的区块生产节点，并且很少被要求证明其能力。 维护顶级节点基础架构成本高昂，且无需证明其能力，预备BP可能会在昂贵的改进和维护上节省开支 - 尤其是当他们没有强制执行的最低要求需满足时。 没有持续的机制来测试预备BP的准备情况，主要BP没有机会进行计划停机，这增加了维护网络的挑战。 无法确定Eos主网在危机时刻是否能够正常运行。
...
答：Telos是一个全新的网络，使用与Eos主网相同的EOSIO操作软件，只是进行了一些小修改。 它不是空投。 然而，TLOS令牌的分布类似于空投，因为任何拥有Eos令牌的人都将根据我们的创世令牌分配算法在其地址接收新的TLOS令牌。

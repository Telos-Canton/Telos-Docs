...
Telos网络有望更加稳定，因为BP是基于能力由广泛的社区投票选举出的，而不是基于少数巨鲸的联盟。 另外，未能生产区块（180个错过的区块）的BP将在30分钟内轮换出来，而不是3个小时。 此外，备用BP将被启用，并且需要每3-7天生成一个块以证明它们时刻准备就绪。 这种半定期计划为BP提供了维护和更新服务器所需的停机时间，从而降低了运营成本。 它还把更多的备用BP加入到治理中，因为备用BP的投票将影响BP投票。 这种方法意味着可以验证备用BP的能力和得到更高的网络弹性（可能减少网络暂停）。

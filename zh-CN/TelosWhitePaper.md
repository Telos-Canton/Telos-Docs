...
Telos将EOSIO代码分配到新的区块链中并更新了以下元素：
...
- Telos代码审查/测试 
  ...
  - 新的代码版本将在主网络激活之前，将在一个专用的测试网络上进行实时测试，这个专用网络由所有BP和预备BP维护。
  - BP和预备BP将根据专用测试网络的结果协调更新。
- Telos DApp开发人员可以将其DApp的原始代码部分指定为专有或开源。 （所有Eos DApps必须是开源的，这将限制许多潜在开发者的参与。）
- Telos将年度通胀率降至2.5％，而不是5％。
- 一旦激活，Telos地址在3年内不使用将不会被销毁。 Eos的这个功能是不公平和不必要的，不会在Telos中实施。 然而，由Eos创世快照创建的创世Telos帐号它们的所有者必须在一年内使用一次，否则帐号将被移除。
...
由通证资助并受委托推广网络的基金会是加密货币中常见的实体。 许多区块链项目为其项目的Telos基金会版本建立了持续的资金。 Telos决定在任何特定时间将这些决定交给BP，以最好地支持Telos网络的去中心化。

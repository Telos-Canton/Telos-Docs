...
Eos网络需要“2/3+1”共识才能运行。 如果没有这个数量的区块生产节点运行，则必须暂停网络，直到2/3+1个区块生产节点处于活动状态。 实际上，这意味着如果现役的21个Eos区块生产节点中只要有6个无法正常出块，那么网络就会崩溃。 预备节点不会自动轮替缺席的区块生产节点。 事实上，当选的BP在至少3个小时内无法被移除。 更加严重的问题是Eos预备BP没有定期测试他们替补和生产区块的能力。 此外，Eos BP没有强制执行的最低要求。 唯一的要求是获得投票。 危险在于，如果BP停工，他们在三小时内将不能被替换，而替换他们的BP可能并没有准备就绪。
